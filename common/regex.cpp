...
static std::string pcre2_error_to_string(int code) {
    ...
    if (ret == PCRE2_ERROR_NOMEMORY) {
        ...
        std::memcpy(&error_message[std::size(error_message) - TRUNCATION_MARKER.length() - 1], TRUNCATION_MARKER.data(),
...
RegexReplaceOnceResult Regex::replace(uint32_t options, std::string_view pattern, std::string_view replace_expression,
        pcre2_match_context *match_context) const {
    ...
    int retval = pcre2_substitute(m_regex.get(), (PCRE2_SPTR8) pattern.data(), pattern.length(),
            /* start_offset */ 0, options,
            /* match_data */ nullptr, match_context, (PCRE2_SPTR8) replace_expression.data(),
            replace_expression.length(), (PCRE2_UCHAR8 *) result.data(), &result_length);
    if (retval == PCRE2_ERROR_NOMEMORY) {
        result.resize(result_length);
        retval = pcre2_substitute(m_regex.get(), (PCRE2_SPTR8) pattern.data(), pattern.length(), 0, options, nullptr,
                nullptr, (PCRE2_SPTR8) replace_expression.data(), replace_expression.length(),
                (PCRE2_UCHAR8 *) result.data(), &result_length);
    }

    if (retval < 0) {
        return RegexReplaceError{
                .error = retval,
        };
    }
...
RegexReplaceAllResult Regex::replace_all(uint32_t options, std::string pattern, std::string_view replace_expression,
        pcre2_match_context *match_context) const {
    RegexReplaceAllResult all_result;

    while (true) {
        RegexReplaceOnceResult once_result = replace(options, pattern, replace_expression, match_context);

        if (auto *error = std::get_if<RegexReplaceError>(&once_result); error != nullptr) {
            all_result = *error;
            break;
        }
...
std::string ag::RegexCompileError::to_string() const {
    return AG_FMT("{} (offset = {})\n"
                  "\t\t{}\n"
                  "\t\t{: >{}}^",
            pcre2_error_to_string(this->error).c_str(), this->offset, this->pattern.c_str(), "", int(this->offset));
}
...
ag::Regex::CompileResult ag::Regex::compile(
        std::string_view regex, uint32_t options, pcre2_compile_context_8 *compile_context) {
    int error = 0;
    PCRE2_SIZE error_offset = 0;
    Pcre2CodePtr compiled(
            pcre2_compile((PCRE2_SPTR8) regex.data(), regex.length(), options, &error, &error_offset, compile_context));
    if (compiled == nullptr) {
...
ag::Regex::MatchResult ag::Regex::match(
        std::string_view text, uint32_t options, size_t start_offset, pcre2_match_context *mcontext) const {
...
std::optional<ag::RegexCompileError> ag::LazyRegex::compile() const {
    if (const auto *uncompiled = std::get_if<Uncompiled>(&this->m_regex); uncompiled != nullptr) {
        Regex::CompileResult compile_result =
                Regex::compile(uncompiled->txt, uncompiled->options, uncompiled->compile_context);
