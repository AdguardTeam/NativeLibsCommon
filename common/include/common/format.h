...
class CountingCompileParseContext : public fmt::detail::compile_parse_context<Char> {
public:
...
class StrictFormatStringChecker {
private:
...
public:
    ...
    FMT_CONSTEXPR void check_num_args() {
        if (context_.next_arg_id_prediction_ != -1 && context_.next_arg_id_prediction_ != context_.num_args()) {
...
public:
    ...
    FMT_CONSTEVAL FMT_INLINE BasicStrictFormatString(const S &s)
            : str_(s) {
        static_assert(fmt::detail::count<(std::is_base_of<fmt::detail::view, fmt::remove_reference_t<Args>>::value
                                && std::is_reference<Args>::value)...>()
                        == 0,
                "passing views as lvalues is disallowed");
        if constexpr (fmt::detail::count_named_args<Args...>() == fmt::detail::count_static_named_args<Args...>()) {
            using checker = StrictFormatStringChecker<Char, fmt::remove_cvref_t<Args>...>;
            checker check(s);
    ...
    BasicStrictFormatString(fmt::runtime_format_string<Char> fmt)
            : str_(fmt.str) {
    }

    FMT_INLINE operator fmt::basic_string_view<Char>() const {
        return str_;
    }
    FMT_INLINE auto get() const -> fmt::basic_string_view<Char> {
        return str_;
    }
};
