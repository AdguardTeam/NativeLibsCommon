...
/**
 * Creates result tuple-like object with error of type ErrString. Result is default initialized or initialized
 * with xs parameters.
...
std::enable_if_t<!has_error<R>, R> forward_error(E&& err, Us&&... xs) {
    return detail::forward_error_impl<R>(
            [&](auto& result) {
                std::get<ErrString>(result) = std::forward<E>(err);
            }, std::forward<Us>(xs)...);
}
 ...
 * @see forward_error
 * @example f_result f() {
 *              static constexpr ag::utils::MakeError<f_result> MakeError;
 *              ...
 *              auto err = g(...);
 *              if (err) {
